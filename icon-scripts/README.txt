...
If you don't have Adobe Illustrator installed, you should be
able to import the AI file with a recent version of Inkscape (>= version 1.2,
from https://inkscape.org/), for viewing purposes.
...
the NetBeans sources, and generate a HTML page summary of all icons. One
snapshot of the generated HTML page summary can be seen at
https://people.csail.mit.edu/ebakke/misc/netbeans-icons-240612.html

The IconTasks script reads the following files as input:
  * icon-scripts/tables/icon-hashes.txt
  ...
  * icon-scripts/tables/ready-artboards.txt
  ...
  * icon-scripts/tables/mappings.tsv
...
* The IconTasks script is a Java application that should be run with
  netbeans-tools/icon-scripts/hidpi-icons as the current working directory. The
  path to a NetBeans source repository (cloned from
  https://github.com/apache/netbeans) must be provided as a command-line
  argument. To work with the script and run it from NetBeans:
  1) Open netbeans-tools/icon-scripts/hidpi-icons as a Maven project in NetBeans
  2) Do a clean build of the project.
  3) Open IconTasks.java (optional, mostly so you can see how script works).
  4) Right-click the hidpi-icons icon in the Projects sidebar, and click
     Properties. Select the "Run" category and put the path to your cloned
     NetBeans repository in "Arguments". Click OK.
  5) Run the project by right-clicking hidpi-icons in the Projects sidebar and
     clicking Run. (Or set it as a main project via Run->Set Main Project and
     press F6.)

Process for Exporting SVG Files from Adobe Illustrator
1) Open the netbeans-tools/icon-scripts/nb_vector_icons.ai file in
   Adobe Illustrator.
...
2) Use File->Save As to save the Illustrator file to a temporary file, to avoid
   accidentally overwriting the original during subsequent steps.
3) Delete the "Old Bitmaps" layer. (This is important, otherwise the old
   bitmap icon may be embedded into each exported SVG file.)
4) From the "File" menu, click "Save a Copy". Use the following settings, and
   click OK:

     File name: icon (will be prefixed to the artboard names; mandatory)
     Folder: netbeans-tools/icon-scripts/tables/illustrator_exports
             (Select the above subfolder in the netbeans-tools repo. If there
             are already SVG files in it from a previous run, delete them
             first.)
     Save as type: SVG
